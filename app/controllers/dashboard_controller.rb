class DashboardController < ApplicationController
  ...
  def index
    ...
    @bad_hosts = Host.with("failed").with("failed_restarts").with("skipped").count
    @active_hosts = Host.with("applied").with("restarted").count
  ...
  def errors
    render :partial => "hosts/minilist", :layout => true, :locals => {
      :hosts => Host.with("failed").with("failed_restarts").with("skipped"),
      :header => "Hosts with errors" }
  end

  def active
    render :partial => "hosts/minilist", :layout => true, :locals => {
      :hosts => Host.with("applied").with("restarted"),
      :header => "Active Hosts" }
  end
